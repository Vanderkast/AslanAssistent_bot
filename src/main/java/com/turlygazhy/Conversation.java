...
public class Conversation {
    ...
    public void handleUpdate(Update update, Bot bot) throws SQLException, TelegramApiException {
        ...
        String inputtedText;//получаемый текст (далее нужен для поиска в базе данных как ключевое значение)

        /*
        * здесь так же идет проверка на нажатие инлайновой кнопки
        * далее следует получение ткста сообщения (от кнопки или введенного текста)
         */
        ...
        try {
            // формирование ответа на запрос по полученному тексту
            ...
            if (command == null) {
                ...
                if(!update.hasCallbackQuery()) {
                    ...
                    sendMessage.setChatId((long) update.getCallbackQuery().getFrom().getId());
                }
