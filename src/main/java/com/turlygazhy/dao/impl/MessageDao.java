...
//    }
    ...
    public void setNewAboutProjectText(String text) throws SQLException{
        PreparedStatement ps = connection.prepareStatement("UPDATE MESSAGE SET TEXT=? WHERE ID=6");
        ps.setString(1,text);
        ps.execute();
    }

    public void setNewGroupRules(String text) throws SQLException{
        PreparedStatement ps = connection.prepareStatement("UPDATE MESSAGE SET TEXT=? WHERE ID=9");
        ps.setString(1, text);
        ps.execute();
    }
}
