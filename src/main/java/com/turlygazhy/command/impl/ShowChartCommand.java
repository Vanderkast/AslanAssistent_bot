...
public class ShowChartCommand extends Command {
    ...
    public boolean execute(Update update, Bot bot) throws SQLException, TelegramApiException {
        ...
        //сообщение пришло от inline кнопки
        ...
        switch (waitingType) {
            ...
            case PERIOD_END: //получение данных о результатах целей (какие цели вообще есть у пользователя)
                try {
                    periodEnd = parse(updateMessageText);
                    //наполняем массив результатов данными
                    ...
                    //что-то типо если данных нет, то выслать error
                    if (results.size() == 0 || results.size() == 1) {
                        sendMessage(44, chatId, bot);
                        waitingType = WaitingType.PERIOD;
                        return false;
                    ...
                    waitingType = WaitingType.CHOOSE_GOAL;
                    //отправка возможных целей
                    sendGoals(chatId, bot);
                    return false;
                } catch (ParseException e) {
                    //если дата была введена в неккоректном формате
                    sendMessage(46, chatId, bot);
                    return false;
                }
            case CHOOSE_GOAL:
    ...
    private void sendReadingChart(Bot bot, Long chatId, List<SavedResult> results, Goal goal) throws TelegramApiException {
        ...
        for (SavedResult result : results) {//добавление результатов относительно даты в таблицу
            chart.addPair(result.getDate(), result.getResult());
        }
        String filePath = chart.getChart(goal.getName());//создание картинки диаграммы и получение полного пути к ней
    ...
    //отправка целей (после чего можно выбрать по какой цели собрать диаграмму
    private void sendGoals(Long chatId, Bot bot) throws SQLException, TelegramApiException {
        ...
        int i = 1;
        for (Goal goal : goals) {//собираем inlint клавиатуру целей
        ...
        keyboard.setKeyboard(rows);//заполняем клавиатуру
    ...
    private void sendChart(Bot bot, Long chatId, List<SavedResult> results, Goal goal) throws SQLException, TelegramApiException {
        ...
        String filePath = chart.getChart(goal.getName());//создание диграммы и получение полного пути к ней
